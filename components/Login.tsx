...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user.role !== role) {
        throw new Error(`This account is for a ${user.role}. Please use the ${user.role} portal.`);
      }
      onLogin(user);
      navigate('/');
    } catch (err) {
  ...
  const title = `Sign in as a ${role.charAt(0).toUpperCase() + role.slice(1)}`;
  ...
  return (
    <AuthLayout title={title}>
      ...
      <div className="mt-6 flex flex-col space-y-4 text-sm text-center">
        <Link to={`/${role}/forgot-password`} className="font-medium text-teal-600 hover:text-teal-500 dark:text-teal-400 dark:hover:text-teal-300">
          Forgot your password?
        </Link>
        <Link to={`/${role}/register`} className="font-medium text-teal-600 hover:text-teal-500 dark:text-teal-400 dark:hover:text-teal-300">
